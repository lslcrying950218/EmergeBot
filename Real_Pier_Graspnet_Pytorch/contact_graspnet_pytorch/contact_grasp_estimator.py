...
class GraspEstimator:
    ...
    def predict_grasps(self, pc, constant_offset=False, convert_cam_coords=True, forward_passes=1):
        ...
        # Keep shapes stable even when only one grasp survives selection.
        # Downstream segment filtering indexes these arrays with boolean/id arrays.
        selected_grasps = pred_grasps_cam[selection_idcs]
        selected_scores = pred_scores[selection_idcs].reshape(-1)
        selected_points = pred_points[selection_idcs].reshape(-1, 3)
        selected_openings = gripper_openings[selection_idcs].reshape(-1)
        return selected_grasps, selected_scores, selected_points, selected_openings
    ...
    def predict_scene_grasps(self, pc_full, pc_segments={}, local_regions=False, filter_grasps=False, forward_passes=1, 
        ...
        verbose_logs = bool(self._contact_grasp_cfg['TEST'].get('verbose_inference_logs', False))
        ...
        if local_regions:
            ...
            for k, pc_region in pc_regions.items():
                pc_region = np.asarray(pc_region, dtype=np.float32)
                if pc_region.ndim == 1:
                    if pc_region.size == 0:
                        pc_region = pc_region.reshape(0, 3)
                    elif pc_region.size % 3 == 0:
                        pc_region = pc_region.reshape(-1, 3)
                    else:
                        if verbose_logs:
                            print('segment {}: invalid 1D region shape {}, skipping'.format(k, pc_region.shape))
                        pred_grasps_cam[k] = np.empty((0, 4, 4), dtype=np.float32)
                        scores[k] = np.empty((0,), dtype=np.float32)
                        contact_pts[k] = np.empty((0, 3), dtype=np.float32)
                        gripper_openings[k] = np.empty((0,), dtype=np.float32)
                        continue
                elif pc_region.ndim != 2 or pc_region.shape[1] != 3:
                    if verbose_logs:
                        print('segment {}: invalid region shape {}, skipping'.format(k, pc_region.shape))
                    pred_grasps_cam[k] = np.empty((0, 4, 4), dtype=np.float32)
                    scores[k] = np.empty((0,), dtype=np.float32)
                    contact_pts[k] = np.empty((0, 3), dtype=np.float32)
                    gripper_openings[k] = np.empty((0,), dtype=np.float32)
                ...
                if pc_region.size == 0:
                    if verbose_logs:
                        print('segment {}: region points=0, skipping empty region'.format(k))
                    pred_grasps_cam[k] = np.empty((0, 4, 4), dtype=np.float32)
                    scores[k] = np.empty((0,), dtype=np.float32)
                    contact_pts[k] = np.empty((0, 3), dtype=np.float32)
                    gripper_openings[k] = np.empty((0,), dtype=np.float32)
                ...
                finite_mask = np.all(np.isfinite(pc_region[:, :3]), axis=1)
                if not np.all(finite_mask):
                    pc_region = pc_region[finite_mask]
                if pc_region.shape[0] == 0:
                    if verbose_logs:
                        print('segment {}: region points=0 after finite filtering, skipping'.format(k))
                    pred_grasps_cam[k] = np.empty((0, 4, 4), dtype=np.float32)
                    scores[k] = np.empty((0,), dtype=np.float32)
                    contact_pts[k] = np.empty((0, 3), dtype=np.float32)
                    gripper_openings[k] = np.empty((0,), dtype=np.float32)
                ...
                if verbose_logs:
                    print('segment {}: region points={}'.format(k, pc_region.shape[0]))
                ...
                if verbose_logs:
                    print('segment {}: grasps before filtering={}'.format(k, len(pred_grasps_cam[k])))
        else:
            if verbose_logs:
                print('using full pc')
        ...
        if filter_grasps:
            ...
            for k in segment_keys:
                ...
                if np.any(pc_segments[k]) and np.any(contact_pts[j]):
                    raw_grasp_count = len(pred_grasps_cam[j])
                    segment_point_count = pc_segments[k].shape[0]
                    ...
                    seg_openings = np.asarray(gripper_openings[j], dtype=np.float32).reshape(-1)
                    if seg_openings.shape[0] == raw_grasp_count:
                        gripper_openings[k] = seg_openings[segment_idcs]
                    else:
                        print(
                            'skipped gripper openings {} (expected {}, got {})'.format(
                                seg_openings, raw_grasp_count, seg_openings.shape[0]
                            )
                        )

                    filtered_grasp_count = len(pred_grasps_cam[k])
                    if verbose_logs:
                        print(
                            'segment {}: points={}, grasps before filtering={}, after filtering={}'.format(
                                k, segment_point_count, raw_grasp_count, filtered_grasp_count
                            )
                        )
                    if local_regions and np.any(pred_grasps_cam[k]) and verbose_logs:
                        print('Generated {} grasps for object {}'.format(filtered_grasp_count, k))
                else:
                    if verbose_logs:
