...
Contact-GraspNet can directly predict a 6-DoF grasp distribution from a raw scene point cloud. However, to obtain object-wise grasps, remove background grasps and to achieve denser proposals it is highly recommended to use (unknown) object segmentation. The Isaac Sim pipeline in this repository now supports generating instance segmaps locally from raw RGB images with SAM before running object-wise grasp prediction. It also supports text-guided target selection on top of those SAM instances, so you can ask for a class like `cup` or `bottle` and restrict grasp planning to the best-matching instance. The generic offline inference path still expects an externally provided `segmap` when you want segmented inference.
